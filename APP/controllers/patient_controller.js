        ...
        if (category !== "students" && category !== "employees") {
            ...
            if (patient === null){
                ...
                if (base === null) {
                    ...
                    if (!doctorRecord.hasOwnProperty('notes')) nullFields.push('notes')
                    if (!doctorRecord.hasOwnProperty('attachments')) {
                        nullFields.push('attachments')
                    } 
                    else {
                        if (!checkArrNull(doctorRecord.attachments)) {
                            let listAtts = doctorRecord.attachments //array of attachments

                            if (Array.isArray(listAtts)) {
                                listAtts.forEach((attachments, index)=> {
                                    if (checkIfNull(attachments.filename)) nullFields.push(`attachment no. ${index}: filename`)
                                    if (checkIfNull(attachments.urlLink)) nullFields.push(`attachment no. ${index}: urlLink`)
                                })
                            } 
                            else {
